                 ...
                 <div className="flex-1 overflow-y-auto p-8 relative scroll-smooth no-scrollbar">
                                ...
                                <div className="space-y-6">
                                    ...
                                    <div className="p-6 rounded-xl relative overflow-hidden bg-deep-1 border border-white/10 group">
                                        ...
                                        <p className="text-[10px] text-gray-400 leading-relaxed mb-4 relative z-10">
                                            Active context injection is enabled. Gemini-3-Pro will utilize these nodes for enhanced reasoning capabilities.
                                        </p>
                                        <button className="w-full py-2.5 bg-purple-500/10 hover:bg-purple-500/20 text-purple-400 border border-purple-500/30 rounded-lg text-xs font-bold transition-all relative z-10 flex items-center justify-center gap-2">
                                            <Brain size={14} /> OPTIMIZE INDEX
                                        </button>
                                    </div>
                                </div>
                             </div>

                             <div className="bg-deep-2/50 border border-white/5 rounded-2xl overflow-hidden mt-6 backdrop-blur-sm">
                                <div className="p-5 border-b border-white/5 flex items-center justify-between bg-deep-1/50">
                                    <h3 className="text-sm font-bold text-white flex items-center gap-2 tracking-wide">
                                        <Database size={16} className="text-cyan-400" /> ACTIVE NEURAL NODES
                                    </h3>
                                    <span className="text-[10px] text-gray-500 font-mono uppercase bg-white/5 px-2 py-1 rounded">Last Sync: {new Date().toLocaleTimeString()}</span>
                                </div>
                                <table className="w-full text-left">
                                    <thead className="bg-deep-1/80 text-[10px] text-gray-500 font-mono uppercase border-b border-white/5">
                                        <tr>
                                            <th className="p-4 pl-6">Node Name</th>
                                    ...
                                    <tbody className="divide-y divide-white/5 text-xs text-gray-300">
                                        {knowledgeBase.length === 0 ? (
                                            <tr>
                                                <td colSpan={6} className="p-12 text-center text-gray-600 italic font-mono">
                                            ...
                                            knowledgeBase.map(node => (
                                                <tr key={node.id} className="hover:bg-white/5 transition-colors group">
                                                    <td className="p-4 pl-6 font-bold text-white flex items-center gap-3">
                                                        <div className="p-1.5 rounded bg-deep-3 border border-white/10 group-hover:border-cyan-500/30 transition-colors">
                                                            {node.type === 'JSON' ? <FileJson size={14} className="text-yellow-400"/> : <FileText size={14} className="text-blue-400"/>}
                                                        </div>
                                                        {node.name}
                                                    </td>
                                                    <td className="p-4 font-mono text-gray-500">{node.type}</td>
                                                    <td className="p-4 font-mono text-gray-400">{(node.size / 1024).toFixed(2)} KB</td>
                                                    <td className="p-4 font-mono text-cyan-300">{node.tokens.toLocaleString()}</td>
                                                    <td className="p-4">
                                                        <span className="flex items-center gap-1.5 text-green-400 bg-green-400/10 px-2 py-0.5 rounded w-fit border border-green-400/20">
                     ...
                     {selectedUser && (
                         <div className="absolute inset-0 z-50 bg-black/80 backdrop-blur-md flex items-center justify-center p-4 animate-fade-in">
                             <div className="bg-deep-1 border border-white/10 rounded-2xl w-full max-w-2xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
                                 <div className="p-6 border-b border-white/5 flex justify-between items-center bg-deep-2">
                                     <h2 className="text-xl font-display font-bold text-white flex items-center gap-2"><Lock size={20} className="text-red-500"/> IDENTITY EDITOR</h2>
                                     <button onClick={() => setSelectedUser(null)} className="text-gray-400 hover:text-white"><X size={24}/></button>
                                 </div>
                                 <div className="p-8 overflow-y-auto flex-1">
                                     <div className="flex flex-col md:flex-row gap-8">
                                         <div className="flex flex-col items-center gap-4">
                                             <div className="relative w-32 h-32 rounded-full p-1 bg-gradient-to-tr from-cyan-500 to-pink-500 group">
                                                 <img 
                                                    src={editForm.photoUrl || `https://ui-avatars.com/api/?name=${editForm.name}`} 
                                                    alt="Avatar" 
                                             ...
                                             <div className="text-center">
                                                 <div className="text-xs font-mono text-cyan-400">NEURAL SIGNATURE</div>
                                                 <div className="h-8 w-32 flex items-center justify-center gap-0.5 mt-1">
                                                     {[...Array(10)].map((_, i) => (
                                                         <div key={i} className="w-1 bg-pink-500 animate-pulse" style={{ height: `${Math.random() * 100}%`, animationDelay: `${i * 0.1}s` }}></div>
                                                     ))}
                                                 </div>
                                             </div>
                                         </div>
                                         <div className="flex-1 space-y-4">
                                             <div className="space-y-1">
                                                 <label className="text-xs font-mono text-gray-500">DISPLAY NAME</label>
                                                 <input 
                                                    type="text" 
                                                    value={editForm.name || ''} 
                                                    onChange={e => setEditForm(p => ({ ...p, name: e.target.value }))}
                                                    className="w-full bg-deep-0 border border-white/10 rounded-lg p-3 text-white focus:border-cyan-500/50 outline-none"
                                                 />
                                             </div>
                                             <div className="space-y-1">
                                                 <label className="text-xs font-mono text-gray-500">QUANTUM ID (EMAIL)</label>
                                                 <input 
                                                    type="text" 
                                                    value={editForm.email || ''} 
                                                    disabled
                                                    className="w-full bg-deep-0 border border-white/10 rounded-lg p-3 text-gray-500 cursor-not-allowed"
                                                 />
                                             </div>
                                             <div className="grid grid-cols-2 gap-4">
                                                 <div className="space-y-1">
                                                     <label className="text-xs font-mono text-gray-500">SUBSCRIPTION PLAN</label>
                                                     <select 
                                                     ...
                                                     >
                                                         ...
                                                         <option value="quantum">Quantum Core</option>
                                                     </select>
                                                 </div>
                                                 <div className="space-y-1">
                                                     <label className="text-xs font-mono text-gray-500">ACCESS ROLE</label>
                                                     <select 
                                                     ...
                                                     >
                                                         ...
                                                         <option value="assistant">Assistant Node</option>
                                                         <option value="creator">System Admin</option>
                                                     </select>
                                                 </div>
                                             </div>
                                         </div>
                                 ...
                                 <div className="p-6 border-t border-white/5 bg-deep-2 flex justify-between items-center">
                                     <button 
                                        onClick={() => handleDeleteUser(selectedUser.id)}
                                        className="px-4 py-2 bg-red-500/10 text-red-400 border border-red-500/20 rounded-lg hover:bg-red-500/20 flex items-center gap-2 text-sm font-bold"
                                     >
                                         <Trash2 size={16} /> TERMINATE LINK
                                     </button>
                                     <div className="flex gap-3">
                                         <button onClick={() => setSelectedUser(null)} className="px-6 py-2 text-gray-400 hover:text-white">Cancel</button>
                                         <button 
